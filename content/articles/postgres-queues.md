...
{{Figure "Number of jobs in queue. One hour into a long-lived transaction, we're at 60k jobs." (ImgSrcAndAltAndClass "/contents/images/postgres-queues/pre-queue-count.png" "Number of jobs in queue. One hour into a long-lived transaction, we're at 60k jobs." "overflowing")}}
...
{{Figure "Median lock time. Normally < 0.01 s, locks are taking 15x longer than that one hour in." (ImgSrcAndAltAndClass "/contents/images/postgres-queues/pre-lock-time.png" "Median lock time. Normally < 0.01 s, locks are taking 15x longer than that one hour in." "overflowing")}}
...
{{Figure "Number of dead tuples in the jobs table. The curve flattens out as jobs get harder to work." (ImgSrcAndAltAndClass "/contents/images/postgres-queues/pre-dead-tuples.png" "Number of dead tuples in the jobs table. The curve flattens out as jobs get harder to work." "overflowing")}}
...
{{Figure "Que finding a job under ideal conditions." (ImgSrcAndAltAndClass "/contents/images/postgres-queues/index.svg" "Que finding a job under ideal conditions." "overflowing")}}
...
{{Figure "Que trying to find a lock in a bloated heap." (ImgSrcAndAltAndClass "/contents/images/postgres-queues/index-bloated.svg" "Que trying to find a lock in a bloated heap." "overflowing")}}
...
{{Figure "Que finding a lock with a greater index specificity despite a bloated heap." (ImgSrcAndAltAndClass "/contents/images/postgres-queues/index-corrected.svg" "Que finding a lock with a greater index specificity despite a bloated heap." "overflowing")}}
...
{{Figure "Number of jobs in the queue with patched version of Que. 30k one hour in." (ImgSrcAndAltAndClass "/contents/images/postgres-queues/post-queue-count.png" "Number of jobs in the queue with patched version of Que. 30k one hour in." "overflowing")}}

And for comparison, here's what it looked like _before_ the patch:

{{Figure "Number of jobs in the queue on vanilla Que. 60k one hour in." (ImgSrcAndAltAndClass "/contents/images/postgres-queues/pre-queue-count.png" "Number of jobs in the queue on vanilla Que. 60k one hour in." "overflowing")}}
