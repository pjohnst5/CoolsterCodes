...
{{Figure "Transactions (tx1, tx2, tx3) mapped to HTTP requests at a 1:1 ratio." (ImgSrcAndAltAndClass "/contents/images/http-transactions/http-transactions.svg" "Transactions (tx1, tx2, tx3) mapped to HTTP requests at a 1:1 ratio." "overflowing")}}
...
{{Figure "A data race causing two concurrent HTTP requests to insert the same row." (ImgSrcAndAltAndClass "/contents/images/http-transactions/concurrent-race.svg" "A data race causing two concurrent HTTP requests to insert the same row." "overflowing")}}
...
{{Figure "An aborted transaction being retried within the same request." (ImgSrcAndAltAndClass "/contents/images/http-transactions/transaction-retry.svg" "An aborted transaction being retried within the same request." "overflowing")}}
