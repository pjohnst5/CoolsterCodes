...
function enableSearch() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const relHref = (0, helper_1.meta)('docfx:rel') || '';
        const worker = new Worker(relHref + 'content/100/javascripts/search-worker.min.js', { type: 'module' });
        ...
        const { lunrLanguages } = yield (0, helper_1.options)();
        ...
        function renderSearchResults(hits, page) {
            ...
            (0, lit_html_1.render)(renderPage(page), document.getElementById('search-results'));
            function renderPage(page) {
                if (hits.length === 0) {
                    return (0, lit_html_1.html) `<div class="search-list">${(0, helper_1.loc)('searchNoResults', { query })}</div>`;
                }
                const start = page * numPerPage;
                const curHits = hits.slice(start, start + numPerPage);
                const items = (0, lit_html_1.html) `
        <div class="search-list">${(0, helper_1.loc)('searchResultsCount', { count: hits.length.toString(), query })}</div>
        <div class="sr-items">${curHits.map(hit => {
            ...
            <div class="sr-item">
              ...
              <div class="item-brief">${mark(itemBrief, query)}</div>
                ...
                return (0, lit_html_1.html) `${items} ${renderPagination()}`;
            }
        ...
        <nav>
          <ul class="pagination">
            <li class="page-item">
              <a class="page-link ${(0, class_map_js_1.classMap)({ disabled: page <= 0 })}" href="#" aria-label="Previous"
                @click="${() => gotoPage(page - 1)}">
                <span aria-hidden="true">&laquo;</span>
              </a>
            </li>
            ${pages.map(i => (0, lit_html_1.html) `
              <li class="page-item">
                <a class="page-link ${(0, class_map_js_1.classMap)({ active: page === i })}" href="#"
                  @click="${() => gotoPage(i)}">${i + 1}</a></li>`)}
            <li class="page-item">
              <a class="page-link ${(0, class_map_js_1.classMap)({ disabled: page >= totalPages - 1 })}" href="#" aria-label="Next"
                @click="${() => gotoPage(page + 1)}">
...
function mark(text, query) {
    ...
    for (let i = 0; i < wordsLower.length; i++) {
        ...
        if (index >= 0) {
            result.push((0, lit_html_1.html) `${text.slice(lastEnd, index)}`);
            result.push((0, lit_html_1.html) `<b>${text.slice(index, index + word.length)}</b>`);
            lastEnd = index + word.length;
        }
    }
    result.push((0, lit_html_1.html) `${text.slice(lastEnd)}`);
