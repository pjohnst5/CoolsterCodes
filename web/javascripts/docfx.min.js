...
`)})}function Go(c){localStorage.setItem("theme",c),c==="auto"?document.documentElement.setAttribute("data-bs-theme",window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light"):document.documentElement.setAttribute("data-bs-theme",c)}async function Xo(){return localStorage.getItem("theme")||(await D()).defaultTheme||"auto"}async function Zo(){Go(await Xo())}function Bo(c){return new MutationObserver(()=>c(Ub())).observe(document.documentElement,{attributes:!0,attributeFilter:["data-bs-theme"]})}function Ub(){return document.documentElement.getAttribute("data-bs-theme")}async function po(c){let t=await Xo(),i=t==="light"?"sun":t==="dark"?"moon":"circle-half";return u`
    <div class='dropdown'>
      ...
      ${G()}`,r)}function g(C){let B=C.map(Z=>{let{href:W,name:x,items:Q,expanded:U}=Z;if(!x)return null;let R=!Q||Q.length<=0,A=R?null:g(Q);if(I!==""&&!A&&!x.toLowerCase().includes(I.toLowerCase()))return null;let y=W?u`<a class='${Rt({"nav-link":!a.includes(Z)})}' href=${W}>${ct(x)}</a>`:R?u`<span class='text-body-tertiary name-only'>${ct(x)}</a>`:u`<a class='${Rt({"nav-link":!a.includes(Z)})}' href='#' @click=${T}>${ct(x)}</a>`,v=I!==""&&U!==!1&&A!=null||U===!0;return u`
    ...
    <ul class='navbar-nav'>${i.map(a=>{if("items"in a){let n=a.items.some(r=>r===b)?"active":null;return u`
      ...
      <ul>${c.map(t=>t.tagName==="H2"?u`<li><a class="link-body-emphasis" href="#${t.id}">${ct(t.innerText)}</a></li>`:u`<li><a class="link-secondary" href="#${t.id}">${ct(t.innerText)}</a></li>`)}</ul>`}function Nn(c){let t=new URL(window.location.href),i,b=0;for(let e of c.map(o=>"items"in o?o.items:o).flat()){if(Wc(e.href))continue;let o=kn(t,e.href);o===b?i=void 0:o>b&&(b=o,i=e)}return i}function kn(c,t){let i=c.pathname.split("/"),b=t.pathname.split("/"),e=0;for(;e<i.length&&e<b.length&&i[e]===b[e];)e++;return e}async function jn(){window.docfx=window.docfx||{};let{start:c}=await D();c?.(),navigator.userAgent.indexOf("docfx/pdf")>=0?await Promise.all([hb(),Qb()]):await Promise.all([Zo(),uo(),Ao(),hb(),i(),Qb()]),window.docfx.ready=!0;async function i(){let[b,e]=await Promise.all([ho(),Qo()]);Vo([...b,...e])}}jn().catch(console.error);
